        ...
        def advice1(convo: Tree, errors: List[AbsTypeError], err: SilentTypeError): List[AbsTypeError] =
          errors.map { e =>
            if (e.errPos samePointAs tree.pos) {
              val header = s"${e.errMsg}%n  Expression does not convert to assignment because:%n    "
              val expansion = s"%n    expansion: ${show(convo)}"
              NormalTypeError(tree, err.errors.flatMap(_.errMsg.lines.toList).mkString(header, s"%n    ", expansion))
            } else e
          }
        def advice2(errors: List[AbsTypeError]): List[AbsTypeError] =
          errors.map { e =>
            if (e.errPos samePointAs tree.pos) {
              val msg = s"${e.errMsg}%n  Expression does not convert to assignment because receiver is not assignable."
